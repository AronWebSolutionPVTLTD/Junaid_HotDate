    ...
    if (!logintype) {
      try {
        ...
        if (!data) {
          ...
          const verificationLink = `https://hot-date.vercel.app/couple/${data._id}`;
      ...
      try {
        ...
        if (exist) {
          ...
          return res
            .status(200)
            .send({ statusCode: 200, Message: token, data: exist });
        } else {
          const data = await userModel.create({
            email: email,
            username: username,
          ...
          if (!data) {
            ...
            res
              .status(201)
              .send({ statusCode: 201, Message: token, data: data });
          }
        }
      } catch (error) {
        console.log(error);
      ...
      if (exist.profile_type == "single") {
        ...
        const data = await userModel.findOneAndUpdate(
          { _id: userId },
          {
            ...req.body,
            interests: JSON.parse(req.body.interests),
            couple: JSON.parse(req.body.couple),
          },
          { new: true }
        );
        console.log(data);
